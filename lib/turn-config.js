...
function getTURNServers() {
    ...
    TURN_PROVIDERS.sort(function (a, b) { return a.priority - b.priority; }).forEach(function (provider) {
        var config = {
            urls: provider.urls
        };
        if (provider.username)
            config.username = provider.username;
    ...
    if (typeof window !== 'undefined') {
        var customTurn = localStorage.getItem('custom_turn_server');
        if (customTurn) {
            try {
                var custom = JSON.parse(customTurn);
                servers.push(custom);
