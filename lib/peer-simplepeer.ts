...
let resolveConnection: ((value: string | null) => void) | null = null; // 保存连接 resolve 函数用于后台恢复
...
function addDebug(msg: string) {
  ...
  if (debugInfo.length > 50) debugInfo.shift(); // 只保留最近 50 条
...
function getSavedId(): string | null {
  try {
    ...
    if (savedId && timestamp) {
      ...
      if (age < maxAge) {
        console.log('[SIMPLEPEER] 📂 从 localStorage 恢复 ID:', savedId, `(年龄: ${Math.floor(age/1000)}秒)`);
        return savedId;
      } else {
...
async function tryConnectWorker(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (savedId) {
      addDebug(`🔄 使用保存的 ID: ${myId}`);
    } else {
      addDebug(`🆕 生成新 ID: ${myId}`);
    }

    addDebug(`正在连接信令服务器: ${workerUrl}`);
    console.log('[SIMPLEPEER] Trying worker:', workerUrl);
    ws = new WebSocket(`${workerUrl}?key=peerjs&id=${myId}&token=token`);
    
    ws.onopen = () => {
      addDebug(`✅ WebSocket 已连接，我的 ID: ${myId}`);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'SIGNAL' && msg.signal) {
          ...
          addDebug(`📥 收到 signal: ${signalType}，来源: ${msg.src}`);
          ...
          try {
            ...
            addDebug(`❌ Signal 应用失败: ${signalErr}`);
...
function setupPeer(
  ...
  p.on('signal', (signal) => {
    ...
    addDebug(`📡 Signal 事件触发: ${signalType}`);

    if (signalType === 'offer') {
      const signalStr = JSON.stringify(signal);
      addDebug(`📤 发送 offer (大小: ${signalStr.length} 字节)`);
    } else if (signalType === 'answer') {
      const signalStr = JSON.stringify(signal);
      addDebug(`📥 收到 answer (大小: ${signalStr.length} 字节)`);
    } else if (signalType === 'candidate') {
      const candidate = (signal as any).candidate;
      if (candidate) {
        // 解析 candidate 获取详细信息
        addDebug(`🧊 ICE candidate 原始数据: ${candidate.substring(0, 150)}...`);
      } else {
  ...
  p.on('connect', () => {
    ...
    if (pc) {
      ...
      iceGatheringTimeout = setTimeout(() => {
        if (candidateCount === 0) {
          addDebug(`⚠️ ICE 收集超时（${isMobile ? 60 : 30}秒），未收集到任何候选`);
          console.warn('[SIMPLEPEER] ICE gathering timeout - no candidates collected');
          addDebug(`🔍 诊断信息：`);
          addDebug(`  - 移动端: ${isMobile}`);
          addDebug(`  - ICE 服务器数量: ${(p as any)._pc?.config?.iceServers?.length || 'unknown'}`);
          addDebug(`  - 网络类型: ${((navigator as any).connection)?.effectiveType || 'unknown'}`);
        }
      ...
      pc.onicecandidate = (event: any) => {
        if (event.candidate) {
          ...
          addDebug(`🧊 ICE 候选 #${candidateCount}: ${type} ${protocol} ${address}:${port}`);
          if (type === 'relay') {
          ...
          addDebug(`✅ ICE 收集完成，共 ${candidateCount} 个候选`);
          addDebug(`📊 候选类型统计：`);
          addDebug(`  - host (本地): ${candidateTypes.host}`);
          addDebug(`  - srflx (STUN): ${candidateTypes.srflx}`);
          addDebug(`  - relay (TURN): ${candidateTypes.relay}`);
          addDebug(`  - prflx (对等反射): ${candidateTypes.prflx}`);
          if (candidateCount === 0) {
  ...
  p.on('error', (err) => {
    ...
    addDebug(`❌ Peer 错误: ${errMsg}`);
    addDebug(`📋 错误堆栈: ${err?.stack || '无堆栈'}`);
